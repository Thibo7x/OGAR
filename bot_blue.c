...
void save_our_sheeps(rencontre *voisins)
{
  ...
  while (pointer != NULL)
  {
    if (!memcmp(pointer->couleur,"\xe6\xf0\xf0",3))
    {
      ...
      {
        if(((distance(pointer->coord.X,pointer->coord.Y, CENTER_PEN_X,CENTER_PEN_Y) > MAP_SIZE_X/10) && distance(pointer->coord.X,pointer->coord.Y, MAP_SIZE_X-CENTER_PEN_X,CENTER_PEN_Y) > MAP_SIZE_X/10) || ((distance(pointer->coord.X,pointer->coord.Y, CENTER_PEN_X,CENTER_PEN_Y) > MAP_SIZE_X/10) && mode == 1))
        {
          while(yellow != NULL)
          {
...
coord intel_blue(rencontre *voisins)
{
  ...
  switch (dog->mode) {
    case 0:
      ...
      //Sortie
      if( count_voisins(saved_sheeps) >= 3 || absolute_rank == max_rank/2)
        dog->mode = 1;
    break;
